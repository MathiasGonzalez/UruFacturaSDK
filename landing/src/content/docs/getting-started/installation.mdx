...
|---------|-------------|--------------|
| **`UruFactura`** | Paquete completo con generación de PDF A4 y térmico (QuestPDF + SkiaSharp + ZXing) | Uso general |
| **`UruFactura.Lite`** | Sin dependencias de PDF. Incluye `ICfePdfGenerator` para inyectar tu propio generador | Microservicios, Azure Functions, entornos con restricciones de peso |
...
<Tabs>
  <TabItem label="Paquete completo">
  ```bash
  dotnet add package UruFactura
  ```
  </TabItem>
  <TabItem label="Paquete Lite">
  ```bash
...
<Aside type="tip">
  Para fijar una versión específica en producción, reemplazá `*` por el número de versión que encontrás en la página de NuGet, por ejemplo `Version="1.0.0"`.
</Aside>
