...
<Aside type="tip">
  Preferí usar `UruFacturaClientBuilder.WithDefaults(config)` en lugar del constructor directo. El builder permite reemplazar cualquier dependencia individualmente, lo que facilita los tests.
</Aside>
...
| `EnviarReporteDiarioAsync` | `Task<RespuestaReporteDiario> EnviarReporteDiarioAsync(DateTime fecha, IEnumerable<Cfe> cfes)` | Envía el reporte diario a DGI. |
...
using var client = UruFacturaClientBuilder.WithDefaults(config)
    .ConGeneradorPdf(miGeneradorPdf)  // opcional
    .Build();

// Tests — reemplazar solo lo que interesa:
using var client = UruFacturaClientBuilder.WithDefaults(config)
    .ConGestorCae(miCaeMock)
...
| `ConGeneradorXml(ICfeXmlBuilder)` | Reemplaza el generador de XML |
| `ConFirmante(ICfeFirmante)` | Reemplaza el firmante digital |
| `ConGestorCae(ICaeManager)` | Reemplaza el gestor de CAEs |
| `ConClienteSoap(IDgiSoapClient)` | Reemplaza el cliente SOAP hacia DGI |
| `ConGeneradorPdf(ICfePdfGenerator?)` | Establece el generador de PDF (puede ser `null`) |
| `ConPdfPorDefecto()` | Agrega el generador PDF predeterminado (solo `UruFacturaSDK`, no Lite) |
...
| `FechaEmision` | `DateTime` | Fecha de emisión (default: `DateTime.Today`) |
...
| `ObtenerAdvertencias` | `IReadOnlyList<string> ObtenerAdvertencias()` | Retorna advertencias de vencimiento/agotamiento |
