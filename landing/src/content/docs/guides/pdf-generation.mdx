...
<Aside type="note">
  La generación de PDF está disponible en el paquete **`UruFactura`** (completo). Si usás **`UruFactura.Lite`**, podés inyectar tu propia implementación de `ICfePdfGenerator` via `UruFacturaClientBuilder.WithDefaults(config).WithPdfGenerator(miGenerador).Build()`. Sin generador configurado, los métodos `GenerarPdfA4` y `GenerarPdfTermico` lanzarán `InvalidOperationException`.
</Aside>
