...
<Aside type="note">
  `CaeManager` es **thread-safe**: podés usarlo desde múltiples hilos concurrentes (p. ej., en una API ASP.NET Core con requests simultáneos) sin necesidad de sincronización adicional.
</Aside>
