...
// ---------------------------------------------------------------------------
// Support loading the certificate from a Base64 env var — in container
// environments this is more convenient than mounting a file.
// Set UruFactura:CertificadoBase64 to the Base64-encoded content of the .p12.
...
if (tempCertPath != null)
{
    ...
    {
        try { File.Delete(tempCertPath); }
        catch (Exception ex) { logger.LogWarning(ex, "No se pudo eliminar el archivo de certificado temporal {Path}.", tempCertPath); }
    });
}
...
{
    app.MapOpenApi();
    app.MapScalarApiReference(o => o.WithTitle("UruFactura Cloudflare API"));
...
// Útil cuando los CAEs se cargan dinámicamente (p.ej. desde una BD externa)
